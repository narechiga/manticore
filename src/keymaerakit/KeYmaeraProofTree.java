	...
	public void implicationRight( int formulaNumber ) 
		...
		try{
			thisImplication = (ImpliesFormula)(
							newNode.sequent.getFormula( formulaNumber ));

		} catch ( Exception e ) {
			e.printStackTrace();
			throw new ManticoreKeYmaeraKitException("Failed to find implication at the specified position");
		}
		newNode.sequent.removeFormula( formulaNumber );
		newNode.sequent.addToAntecedent( thisImplication.getAntecedent() );
		newNode.sequent.addToSuccedent( thisImplication.getSuccedent() );
