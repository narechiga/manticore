...
addpath( genpath( getenv('YALMIP')));
addpath( genpath( getenv('SEDUMI')));
eval(sprintf('javaaddpath %s', getenv('MANTICORE')))
...
while optim_cost<0&&iter<=max_iter
    ...
    if true
        ...
        if true
            ...
            seed_for_optimizer = rand(s,2,10)*2*search_radius-search_radius;
        ...
        nmoptimize(@cost,seed_for_optimizer(:,ind), [], [],[], [],[],[], @search_constr_func, 'superstrict', OPTIONS, 'fminsearch' ,Pout,P,alph,sdpvector,sdpindets);
